...
export default function NewArticleForm({ navLinks, defaultType }: { navLinks: NavLink[]; defaultType: string }) {
    ...
    <div className="w-full flex flex-col gap-8 max-w-4xl mx-auto pb-48">
      ...
      <form action={saveArticle} className="flex flex-col gap-6">
        ...
        <div className="flex gap-3">
          {navLinks.filter(n => n.type).map(n => (
            <label key={n.id} className="flex items-center gap-2 cursor-pointer">
              <input type="radio" name="type" value={n.type!} defaultChecked={n.type === defaultType} className="accent-neutral-900" />
              <span className="font-sans text-sm capitalize text-neutral-600">{n.label}</span>
            </label>
          ))}
        </div>
