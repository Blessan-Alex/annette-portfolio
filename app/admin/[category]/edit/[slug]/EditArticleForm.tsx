...
export default function EditArticleForm({ article }: { article: any }) {
  const [content, setContent] = useState(article.content_data?.html || '')
  const [thumbnailUrl, setThumbnailUrl] = useState(article.thumbnail_url || '')
  const [thumbnailPreview, setThumbnailPreview] = useState(article.thumbnail_url || '')
  ...
  return (
    <div className="w-full flex flex-col gap-8 max-w-4xl mx-auto pb-48">
      <header>
        <h1 className="font-serif text-3xl md:text-[42px] text-on-surface">Edit Article</h1>
        <p className="font-sans text-neutral-500 text-sm">Modifying an existing entry...</p>
      </header>
      ...
      <form action={saveArticle} className="flex flex-col gap-6">
        ...
        <div className="flex flex-col md:flex-row items-start gap-4 md:gap-6 p-4 bg-neutral-50 rounded-2xl border border-neutral-100">
          ...
          <div className="flex flex-col gap-2">
            ...
            {thumbnailPreview && (
              <button type="button" onClick={() => { setThumbnailUrl(''); setThumbnailPreview('') }} className="font-sans text-xs text-red-400 hover:text-red-600 w-fit">
                Remove cover
              </button>
            )}
          </div>
        ...
        <div className="w-full mt-4 editor-container">
          <Editor initialContent={content} onChange={(html) => setContent(html)} />
        </div>
        ...
        <div className="fixed top-5 right-4 md:top-auto md:bottom-8 md:right-8 flex gap-2 md:gap-3 z-[60]">
          ...
          <button type="submit" className="px-4 md:px-5 py-2 md:py-2.5 rounded-full bg-neutral-900 text-white hover:bg-neutral-800 transition-colors shadow-md font-sans text-xs md:text-sm font-medium">Update</button>
        </div>
      </form>
    </div>
  )
